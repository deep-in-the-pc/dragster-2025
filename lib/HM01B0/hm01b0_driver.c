...
int hm01b0_init(const struct hm01b0_config* config)
{
    ...
    sm_config_set_in_pin_count(&hm01b0_inst.pio_sm_config, config->data_bits);
    ...
    //gpio_pull_up(config->vsync_pin);
    //gpio_pull_up(config->hsync_pin);
    //gpio_pull_up(config->pclk_pin);
    for(int i = 0; i < config->data_bits; i++) {
        //gpio_pull_up(config->data_pin_base + i);
    }
    //hm01b0_write_reg8(0x0100, 0x01); // MODE_SELECT (start streaming)
    return 0;
}
...
void hm01b0_read_frame(uint8_t* buffer, size_t length)
{
    ...
    hm01b0_write_reg8(0x0100, 0x00); // MODE_SELECT (stop streaming)
}
...
/* --- New Function: Set Data Interface --- */
int hm01b0_set_data_interface(uint8_t data_bits)
{
    ...
    hm01b0_write_reg8(0x3059, bit_control_val);
    hm01b0_inst.config.data_bits = data_bits;
    return 0;
}

/* --- New Function: Set Resolution --- */
int hm01b0_set_resolution(uint16_t width, uint16_t height)
{
    ...
    hm01b0_inst.config.width = width;
    hm01b0_inst.config.height = height;
    return 0;
}
